...
const UpdateItems = () => {
  ...
  const { _id } = item;
  console.log("item loader from update", item);
  ...
  const onSubmit = async (data) => {
    ...
    if (res.data.success) {
      ...
      const menuResponse = await axiosHook.patch(`/menu/${_id}`, menuItem);
      console.log(menuResponse.data);
      if (menuResponse.data.modifiedCount > 0) {
        Swal.fire("Nice", `${item.name} updated`, "successs");
      }
    }
  };
  return (
    <div>
      <SectionTitle
        title={"Update Your Items"}
        heading={" Update what you want"}
      ></SectionTitle>
      <div className="p-4">
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex gap-6">
            ...
            <div className="form-control w-full">
              ...
              <select
                ...
                className="input border border-yellow-500 focus:outline-none focus:border-2 focus:border-yellow-500"
              >
                <option value="salad" disabled selected>
                  Select Category
                </option>
          ...
          <input
            ...
            {...register("image")}
            type="file"
            className="file-input file-input-bordered file-input-warning w-full  my-3"
          />
